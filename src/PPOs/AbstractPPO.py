...
class AbstractPPO(metaclass=ABCMeta):
    ...
    def rollout_episode(self,
        ...
        for _ in range(self.timestep_per_episode):
            ...
            to_buffer.append((state.detach(), action.detach(), log_prob.detach(), value.detach(), reward.detach(), mask.detach(), done.detach()))
            state = next_state
        ...
        return best_reward, average_reward, to_buffer


    def rollout_episodes(self):
        self.writer = None
        num_processes = mp.cpu_count()
        with mp.Pool(num_processes) as pool:
            results = pool.map(self.rollout_episode, (range(num_processes)))
            best_rewards, average_rewards,to_buffer = zip(*results)

        writer = SummaryWriter()

        for i, (best_reward, average_reward,to_buffer) in enumerate(results):

            for state, action, log_prob, value, reward, mask, done in to_buffer:
            ...
            writer.add_scalar(f'Process {i}/Best Reward', best_reward, self.current_episode)
            writer.add_scalar(f'Process {i}/Average Reward', average_reward, self.current_episode)

        self.buffer.compute_advantages()
        self.writer = SummaryWriter()
        return max(best_rewards), sum(average_rewards) / len(average_rewards)
