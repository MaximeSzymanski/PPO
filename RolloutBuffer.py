...
class RolloutBuffer():
    ...
    It has several lists to store the data from the environment

    Attributes:
        rewards: List[torch.Tensor] : rewards from the environment
        values: List[torch.Tensor] : values from the critic
        log_probs: List[torch.Tensor] : log probabilities from the actor
        actions: List[torch.Tensor] : actions from the actor
        dones: List[torch.Tensor] : dones from the environment
        states: List[torch.Tensor] : states from the environment
        advantages: List[torch.Tensor] : advantages from the environment
        masks: List[torch.Tensor] : masks to indicate the end of the episode. Used to calculate the returns
        returns: List[torch.Tensor] : returns (i.e. discounted rewards) from the environment
        minibatch_size: int : size of the minibatch
        gamma: float : discount factor
        gae_lambda: float : lambda factor for Generalized Advantage Estimation
